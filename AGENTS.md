...
- **Extract** (`extract/`) — dispatch by extension with lazy imports (Markdown/`text.py`,
  ...
  images/`image.py` via Tesseract with optional Codex escalation, code verbatim via `text.py`).
...
- `GarageMCPService` owns a separate long-lived `garage-mcp` HTTP process at
  `127.0.0.1:8787/mcp`; Codex Desktop/Code instead spawn their own stdio `garage-mcp` via `garage
