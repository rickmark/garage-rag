...
Modified for garage_rag: absl logging replaced by the standard library's;
...
from garage_rag.enrich.langextract import chunking
from garage_rag.enrich.langextract import prompting
from garage_rag.enrich.langextract import resolver as resolver_lib
from garage_rag.enrich.langextract import base_model
from garage_rag.enrich.langextract import data
from garage_rag.enrich.langextract import exceptions
...
class Annotator:
  ...
  def _annotate_documents_single_pass(
    ...
    # garage_rag: no progress bar (tqdm); batches are iterated directly.
    batch_iter = batches
  ...
  def annotate_text(
    ...
    if debug and annotations[0].extractions:
      ...
      logging.debug(
          "Extracted %d entities (%d unique types) from %d chars in %d"
          " chunks, %s s",
          num_extractions,
          unique_classes,
          len(text),
