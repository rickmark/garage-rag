...
as an arm64 binary; ICU and zlib come from the
libraries under `//ext` and line editing from the SDK's libedit (not GPL-3.0
GNU Readline), so the result still depends on nothing but macOS
