...
> **Status (1.5 alpha).** None of the milestones below (§8) has landed yet: there is no provider
> registry, enrichment queue, `dream` worker, memory, categories or triples in the code. What the
> 1.5 alpha carries instead is configurable, multi-prompt fact extraction (`facts.prompts`,
> `fact_runs`, `enrich-facts --stale-only`), mail and Messages extraction, cheaper re-ingest
> (`ingest_outcomes`, stat skips, chunk reuse), in-process Tesseract OCR, Apache AGE in the bundled
> Postgres, the App Group data folder and keychain, the launcher helper bundles, Sparkle, and the
> redesigned app. The migration numbers this plan proposes (`008_enrich_queue.sql`,
> `010_categories.sql`, …) have since been taken: the schema runs to `013_fact_prompts.sql`, so
> anything built from this plan starts at `014`. See `docs/architecture.md` and `docs/schema.md`
> for what exists.
