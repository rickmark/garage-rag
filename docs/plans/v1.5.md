...
CREATE TYPE enrich_job_kind AS ENUM (
    ...
    'canonicalize_entities', 'anticipate_questions', 'summarize_communities',
...
- **Dependency order inside a session**: summaries → chunk contexts → categories → facts →
  document linking → entity canonicalization → anticipated questions → community summaries →
...
4. **Cross-document dedup → canonical facts.** New columns:
   ...
   ALTER TABLE facts ADD COLUMN status text NOT NULL DEFAULT 'kept';  -- kept | merged | dropped | dormant
...
- **BUILD**: `py_test` entries for the eleven new test files; `aspect gazelle` for new modules.
...
| M5 | **Dreaming** | `IdleMonitor` + scheduler in the app, `consolidate_memories`, `synthesize_insights` and `deep_thoughts` (probes) passes, Dreaming status + Dream journal | M2, M3, M4, M4b |
